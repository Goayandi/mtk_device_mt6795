...
#include "cac.h"
#define FPC1021_WIDTH   160
#define FPC1021_HEIGHT  160

int32_t fpc1021_module_close(hw_device_t* device)
{
    if (device) {
        preprocessor_cleanup();
        fpc102x_device_t* fpc1021device = (fpc102x_device_t*) device;
        calibrationDestroy(fpc1021device->cac_calibration);
        delete fpc1021device->raw_image.buffer;
        delete fpc1021device->raw_image_fallback.buffer;
        delete fpc1021device->driver;
        delete fpc1021device;
    }
    return 0;
}

int fpc1021_module_open(const hw_module_t* module, const char* name,
        hw_device_t** device)
{
    *device = NULL;
    int status = 0;
...
#ifdef USE_FPC2050
    this_sensor_type = FPC102X_SENSOR_TYPE_FPC1025;
#else
    this_sensor_type = FPC102X_SENSOR_TYPE_FPC1021;
#endif

    fpc102x_device_init(dev, FPC1021_WIDTH, FPC1021_HEIGHT, this_sensor_type);
    ...
    dev->device.common.close = fpc1021_module_close;
...
static struct hw_module_methods_t fpc1021_module_methods = {
    fpc1021_module_open
};
...
fpc_fingerprint_moudule_t HAL_MODULE_INFO_SYM = {
    {
        ...
        "FPC1021 Touch Sensor",
        "Fingerprint Cards AB",
        &fpc1021_module_methods,
    },
};
